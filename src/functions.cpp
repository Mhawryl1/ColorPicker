...
struct sRGB
{
    explicit sRGB(COLORREF cRef) : color(cRef){};
    union {
        COLORREF color;
        struct
        {
    ...
    [[nodiscard]] std::wstringstream GetHex() const
    {
        wstringstream ss; // NOLINT
        if (r < 16) {
            ss << std::hex << 0 << r;
        }
        else {
...
std::wstring ColorToRGB(COLORREF cRef)
{
    ...
    return std::wstring{std::to_wstring(rgb.r) + L", " + std::to_wstring(rgb.g) + L", " + std::to_wstring(rgb.b)};
}
...
std::wstring ColorToHSV(COLORREF cRef)
{
    ...
    auto r     = static_cast<float>(rgb.r / 255.0F);
    auto g     = static_cast<float>(rgb.g / 255.0F);
    auto b     = static_cast<float>(rgb.b / 255.0F);
    ...
    return std::wstring{std::to_wstring((short)h) + L"\u00BA   " + std::to_wstring((short)s) + L"%   " +
                        std::to_wstring((short)v) + L"%"};
}

HFONT MakeFont(int size, LPCSTR family, int width, int weight)
{
    return CreateFontA(size, width, 0, 0, weight, FALSE, FALSE, FALSE, ANSI_CHARSET, OUT_DEFAULT_PRECIS,
                       CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_SWISS, family);
}
bool DrawIconFormResource(HWND hwnd, HBRUSH hBrush, int bkgColor, unsigned int ICON, int width, int height)
{
    ...
    if (!hIcon) {
        MessageBoxW(NULL, L"Load icon from resource failed!", L"Error", MB_ICONERROR | MB_OK);
        return false;
    }
    HDC hdc = GetDC(hwnd);
    RECT rct;
    ...
    DrawIconEx(hdc, rct.left + 5, rct.top + 5, hIcon, width, height, 0, NULL, DI_NORMAL);
    DeleteObject(hBrush);
    ReleaseDC(hwnd, hdc);
    return true;
}
