...
int CALLBACK WinMain(HINSTANCE hInstance,
  ...
  pWin->CreateSysTrayIcon(
      pWin->GetParentHWND(), IDI_PICKER, WM_USER_SHELLICON, L"Color Picker");

  HWND hCopyHEX =
      pWin->CreateChildWindow(CopyPasteProc,
                              Window::GetWindowHandel(L"BigColorPanel"),
                              L"CopyPaste",
                              L"CopyPasteHEX",
                              {335, 110, 35, 35},
                              NULL,
  ...
  pWin->CreateChildWindow(BarMenuProc,
                          Window::GetWindowHandel(L"BigColorPanel"),
                          L"BarMenuClass",
                          L"MiniButtons",
                          {325, 10, 30, 30},
  ...
  while (pWin->ProcessMessage()) {
    if (GetCursorPos(&mPoint) && IsWindowVisible(pWin->GetParentHWND()) &&
      ...
      wPoint.x =
          mPoint.x + 260 > screenSize.right ? screenSize.right - 260 : mPoint.x;
      wPoint.y =
          mPoint.y + 90 > screenSize.bottom ? screenSize.bottom - 90 : mPoint.y;
      pWin->ChangeWinPos(pWin->GetParentHWND(), wPoint, pWin->GetWinRect());

      cRef = GetPixel(hdc, mPoint.x, mPoint.y);
      SendMessage(
          pWin->GetParentHWND(), WM_COMMAND, WM_SENDCOLOR, (LPARAM)cRef);
    ...
    if (((GetKeyState(VK_CONTROL) < 0 && GetKeyState(0x43) < 0) ||
        ...
        (!IsWindowVisible(Window::GetWindowHandel(L"BigColorPanel")) &&
         IsWindowVisible(pWin->GetParentHWND()) &&
         !IsIconic(pWin->GetParentHWND()))) {
      ...
      SendMessage(Window::GetWindowHandel(L"BigColorPanel"),
                  WM_COMMAND,
                  WM_SENDCOLOR,
                  (LPARAM)cRef);
      SendMessage(Window::GetWindowHandel(L"PickedColorBigPanel"),
                  WM_SETBKGCOLOR,
                  NULL,
                  (LPARAM)cRef);
      Window::m_sCref = cRef;
