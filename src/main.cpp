...
int CALLBACK WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR pCmdLine, int nCmdShow)
{
    ...
    HDC hdc       = GetDC(NULL);
    COLORREF cRef = 0;
    auto *pWin    = new Window({120, 120, 260, 90});                            // NOLINT
    pWin->CreateNewWindow(L"ColorPanel", L"BigColorPanel", {30, 30, 400, 290}); // NOLINT
    pWin->CreateSysTrayIcon(pWin->GetParentHWND(), IDI_PICKER, WM_USER_SHELLICON, L"Color Picker");
    HWND hCopyHEX = pWin->CreateChildWindow(CopyPasteProc, Window::GetWindowHandel(L"BigColorPanel"), L"CopyPaste",
                                            L"CopyPasteHEX", {335, 110, 35, 35}, NULL, (LPWSTR)IDC_HAND,
                                            RGB(55, 55, 55), NULL, WS_VISIBLE | WS_CHILD);
    pWin->CreateRoundedWindow(hCopyHEX, 5, 5);
    HWND hCopyRGB = pWin->CreateChildWindow(Window::GetWindowHandel(L"BigColorPanel"), L"CopyPaste", L"CopyPasteRGB",
                                            {335, 175, 35, 35}, NULL, WS_VISIBLE | WS_CHILD);
    pWin->CreateRoundedWindow(hCopyRGB, 5, 5);
    HWND hCopyHSV = pWin->CreateChildWindow(Window::GetWindowHandel(L"BigColorPanel"), L"CopyPaste", L"CopyPasteHSV",
                                            {335, 235, 35, 35}, NULL, WS_VISIBLE | WS_CHILD);
    pWin->CreateRoundedWindow(hCopyHSV, 5, 5);

    pWin->CreateChildWindow(BarMenuProc, Window::GetWindowHandel(L"BigColorPanel"), L"BarMenuClass", L"MiniButtons",
                            {325, 10, 30, 30}, NULL, (LPWSTR)IDC_HAND, RGB(55, 55, 55), NULL, WS_VISIBLE | WS_CHILD);
    pWin->CreateRoundedWindow(Window::GetWindowHandel(L"MiniButtons"), 5, 5);
    pWin->CreateChildWindow(Window::GetWindowHandel(L"BigColorPanel"), L"BarMenuClass", L"CloseButton",
                            {358, 10, 30, 30}, NULL, WS_VISIBLE | WS_CHILD);
    pWin->CreateRoundedWindow(Window::GetWindowHandel(L"CloseButton"), 5, 5);
    // pWin->CreateChildWindow(Window::GetWindowHandel(L"BigColorPanel"),
    // L"BarMenuClass", L"OpenPicker", {45, 40, 30, 30}, NULL, WS_VISIBLE |
    // WS_CHILD);
    ...
    while (pWin->ProcessMessage()) {
        ...
        if (((GetKeyState(VK_CONTROL) < 0 && GetKeyState(0x43) < 0) || GetKeyState(VK_LBUTTON) < 0) &&
            ...
            // wPoint.x = mPoint.x + 400 > screenSize.right ? screenSize.right - 500 :
            // mPoint.x; wPoint.y = mPoint.y + 290 > screenSize.bottom ?
            // screenSize.bottom - 350 : mPoint.y; RECT rct;
            // GetWindowRect(Window::GetWindowHandel(L"BigColorPanel"), &rct);
            // pWin->ChangeWinPos(Window::GetWindowHandel(L"BigColorPanel"), wPoint,
            // rct);
        }
        Sleep(10);
    }
