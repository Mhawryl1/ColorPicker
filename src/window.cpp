    ...
    case WM_USER_SHELLICON: {
      switch (lParam) {
        ...
        case WM_RBUTTONDOWN: {
          ...
          if (!hIClose)
            hIClose = (HBITMAP)LoadImage(GetModuleHandle(NULL),
                                         MAKEINTRESOURCE(IDI_CLOSEBMPR),
                                         IMAGE_BITMAP,
                                         20,
                                         20,
                                         LR_COPYFROMRESOURCE);
          if (!hISetting)
            hISetting = (HBITMAP)LoadImage(GetModuleHandle(NULL),
          ...
          SetMenuItemBitmaps(
              hMenu, IDM_SETTING, MF_BITMAP, hISetting, hISetting);
          SetMenuItemBitmaps(hMenu, IDM_CLOSE, MF_BITMAP, hIClose, hIClose);
          TrackPopupMenu(hMenu,
                         TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_HORNEGANIMATION,
                         pt.x,
                         pt.y,
                         0,
                         hwnd,
...
LRESULT CALLBACK BigPanelColorProc(HWND hwnd,
                                   UINT uMsg,
                                   WPARAM wParam,
                                   LPARAM lParam) {
  static HBRUSH hBrush;
...
LRESULT CALLBACK ColorPanelProc(HWND hwnd,
                                UINT uMsg,
                                WPARAM wParam,
                                LPARAM lParam) {
  ...
  switch (uMsg) {
    ...
    case WM_DESTROY:
      ...
      break;
    case WM_CREATE: {
      hWndTxHEX = CreateWindow(L"STATIC",
                               L"HEXCOLOR",
                               WS_VISIBLE | WS_CHILD,
                               30,
                               120,
      ...
      hWndTxRGB = CreateWindow(L"STATIC",
                               L"RGBCOLOR",
                               WS_VISIBLE | WS_CHILD,
                               30,
                               180,
      ...
      hWndTxHSV = CreateWindow(L"STATIC",
                               L"HSVCOLOR",
                               WS_VISIBLE | WS_CHILD,
                               30,
                               240,
    ...
    case WM_DRAWITEM: {
      ...
      if (ds->hwndItem == hButton) {
        ...
        HICON hIcon =
            (HICON)LoadIcon(GetModuleHandle(0), MAKEINTRESOURCE(IDI_PICKER));
        if (!hIcon) {
    ...
    case WM_CTLCOLORSTATIC: {
      HDC hdcStatic = (HDC)wParam;
      if (lParam == (LPARAM)hWndTxRGB || lParam == (LPARAM)hWndTxHEX ||
...
LRESULT CALLBACK BarMenuProc(HWND hwnd,
                             UINT uMsg,
                             WPARAM wParam,
                             LPARAM lParam) {
...
LRESULT CALLBACK CopyPasteProc(HWND hwnd,
                               UINT uMsg,
                               WPARAM wParam,
                               LPARAM lParam) {
  static HBRUSH hBrush = NULL;
  ...
  switch (uMsg) {
    case WM_CREATE: {
      RECT rct;
      GetClientRect(hwnd, &rct);
      hIcon = (HICON)LoadImage(GetModuleHandle(0),
                               MAKEINTRESOURCE(IDI_PASTEWHITE),
                               IMAGE_ICON,
                               25,
                               25,
                               LR_COPYFROMRESOURCE);
      hIconWht = (HICON)LoadImage(GetModuleHandle(0),
...
Window::Window(const RECT& rc)
    : hInstance(GetModuleHandle(NULL)), mRect(rc), m_cRef(0) {
  ...
  m_hwNdChild = CreateWindowW(L"Color child window",
                              NULL,
                              WS_CHILD | WS_VISIBLE,
                              cRect.left,
                              cRect.top,
...
HWND Window::CreateNewWindow(const wchar_t* className,
  ...
  HWND hwNdChild = CreateWindowW(L"Color_window",
                                 NULL,
                                 WS_CHILD | WS_VISIBLE,
                                 cRect.left,
                                 cRect.top,
  ...
  if (!hwNdChild) {
    MessageBoxW(
        NULL, L"Create widnow failed!", L"Error!", MB_OK | MB_ICONERROR);
    return 0;
  }
...
HWND Window::GetWindowHandel(const wchar_t* wndName) {
  if (!m_uMap.contains(wndName)) {
    MessageBoxW(
        NULL, L"Invalid handel name!", L"Warning", MB_ICONWARNING | MB_OK);
    return 0;
  }
  return m_uMap[wndName];
}

HWND Window::CreateChildWindow(WNDPROC WndProc,
                               HWND hwnd,
  ...
  if (hWND == NULL) {
    ...
    MessageBox(NULL,
               (std::wstring(L"Creating window failed! Error code: ") +
...
HWND Window::CreateChildWindow(HWND hwnd,
                               const wchar_t* className,
                               const wchar_t* windowTitle,
                               const RECT& pos,
                               HMENU hMenu,
  ...
  if (!wcslen(buffer)) {
    MessageBox(
        NULL, L"Class name is required!", L"Error", MB_ICONERROR | MB_OK);
  }
  HWND hWND = CreateWindowEx(NULL,
                             buffer,
                             windowTitle,
  ...
  if (hWND == NULL) {
    MessageBox(
        NULL, L"Creating window failed!", L"Error", MB_ICONERROR | MB_OK);
    return 0;
  }
  Window::m_uMap[windowTitle] = hWND;
  UpdateWindow(hWND);
...
bool Window::CreateRoundedWindow(HWND hwnd, short w, short h) {
  ...
  HRGN hRgn = CreateRoundRectRgn(
      0, 0, rgn.right - rgn.left, rgn.bottom - rgn.top, w, h);
  if (hRgn) {
...
bool Window::CreateSysTrayIcon(HWND hwnd,
                               int IDI_ICON,
                               int WM_CALLBACK,
                               const wchar_t* tip) {
  NOTIFYICONDATA nd{0};
  ...
  nd.uID = IDI_ICON;
  nd.uFlags = NIF_MESSAGE | NIF_ICON | NIF_TIP | NIF_STATE | NIF_GUID |
